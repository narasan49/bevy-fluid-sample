...
use crate::{
    euler_fluid::{
        advection::{AdvectionBindGroup, AdvectionMaterial, AdvectionPipeline},
        uniform::{SimulationUniform, SimulationUniformBindGroup},
    },
    texture::ImageForCS,
};
use bevy::{
    ...
    render::{
        ...
        render_resource::{
            CachedPipelineState, ComputePassDescriptor, PipelineCache, TextureFormat,
        },
...
fn update(mut query: Query<&mut SimulationUniform>) {
        ...
        match self.state {
            ...
            AdvectionState::Update => {
                ...
                let swap_pipeline = pipeline_cache
                    .get_compute_pipeline(pipeline.swap_pipeline)
                    .unwrap();
